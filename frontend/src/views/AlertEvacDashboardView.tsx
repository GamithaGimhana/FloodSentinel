...
import { scenarioFromDistrict, probabilityChange, signed, dateLabel } from '../services/assessment';
...
export default function AlertEvacDashboardView({ district, reading, onDistrict }: {
  ...
  return <div className="simulation-layout">
    <section className="card scenario-card"><div className="card-heading"><div><span className="eyebrow">EXPLORE THE VARIABLES</span><h2>What if conditions change?</h2></div><FlaskConical size={24} /></div>
      <p className="muted">Start with {district.name}’s assessment and change the inputs to test the model’s response. Rainfall changes also update the monthly-rainfall proxy; soil changes update drainage and water-index proxies.</p>
      ...
      <div className="preset-row"><button onClick={() => setParams({ ...baseline.inputs, rainfall_7d_mm: Math.min(5000, baseline.inputs.rainfall_7d_mm * 1.25) })}>25% more rainfall</button><button onClick={() => setParams({ ...baseline.inputs, rainfall_7d_mm: baseline.inputs.rainfall_7d_mm * .75 })}>25% less rainfall</button><button onClick={() => setParams({ ...baseline.inputs, soil_saturation_pct: 85 })}>Saturated soil</button></div>
      ...
      {fields.map(f => <div className="scenario-field" key={f.key}><div><label htmlFor={f.key}>{f.label}</label><output htmlFor={f.key}>{Number(params[f.key].toFixed(2))} <small>{f.unit}</small></output></div><input id={f.key} type="range" min={Math.min(f.min, baseline.inputs[f.key])} max={Math.max(f.max, baseline.inputs[f.key], params[f.key])} step="any" value={params[f.key]} onChange={e => setParams({ ...params, [f.key]: Number(e.target.value) })} /><p>{f.hint} · Starting value: {baseline.inputs[f.key]} {f.unit}</p></div>)}
