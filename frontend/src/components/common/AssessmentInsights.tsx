...
export function AssessmentHeadline({ name, reading, loading, onExplore }: {
  ...
  return <section className="assessment-headline" aria-label={`${name} assessment summary`}>
    <div><span className="eyebrow">YOUR DISTRICT / {name.toUpperCase()}</span>
      ...
      <p>{prediction ? `Model probability ${(prediction.flood_probability * 100).toFixed(2)}% · decision threshold ${(prediction.threshold_used * 100).toFixed(2)}%.` : 'Weather and a loaded model are needed to assess this district.'} This is a research assessment, not a verified flood forecast.</p>
...
export default function AssessmentInsights({ reading, refresh }: { reading: Telemetry; refresh: number }) {
  ...
  return <div className="insights-grid">
    ...
    <section className="card"><span className="eyebrow">BEHIND THE ASSESSMENT</span><h2>Know where the inputs come from</h2><p className="muted">Only rainfall comes directly from the weather provider into this model. Drainage and water-index values are proxies; most local features are still assumptions.</p>
      <div className="provenance-list"><div><strong>Weather estimate</strong><span>Seven completed days of precipitation</span></div><div><strong>Derived inputs</strong><span>Monthly rainfall, drainage and water-index proxies</span></div><div><strong>District references</strong><span>District coordinates and reference elevation</span></div><div><strong>Demonstration values</strong><span>River distance, land cover, flood history and other local features</span></div></div>
