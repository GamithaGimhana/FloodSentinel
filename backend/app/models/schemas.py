...
class EngineeredFeatures(BaseModel):
    ...
    drainage_saturation_ratio: float = Field(..., description="rainfall_7d * (1 - drainage_index)")
    water_veg_contrast: float = Field(..., description="ndwi - ndvi")
    river_proximity_buffer: float = Field(..., description="exp(-distance_to_river / 1000)")
    log_population_density: float = Field(..., description="log(pop_density + 1)")
    runoff_vulnerability: float = Field(..., description="(built_up_percent / 100) / (drainage_index + 0.01)")


class PredictionResponse(BaseModel):
    """Complete flood prediction result with alert categorization."""
    flood_probability: float = Field(..., description="Model probability or unvalidated heuristic score (0.0 to 1.0)")
    ...
    recommended_action: str = Field(..., description="Educational interpretation; not an official warning")
...
class SimulationRequest(BaseModel):
    ...
    elevation_m: float = Field(default=50.0, ge=0, le=2500, description="Elevation slider (m)")
    ...
    monthly_rainfall_mm: Optional[float] = Field(default=None, ge=0, le=3000, description="Override monthly rainfall")
    ndvi: Optional[float] = Field(default=None, ge=-1, le=1, description="Override NDVI")
    ndwi: Optional[float] = Field(default=None, ge=-1, le=1, description="Override NDWI")
